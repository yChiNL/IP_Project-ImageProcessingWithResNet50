...
# 預測
def predice(image, model, preprocess, weights, device):
...
# 主處理函數
def process_image(image_path, center, radius, model, preprocess, weights, output_path, device):
    ...
    for img, label in [(original_image, "original_image"),
        ...
        category, score = predice(img, model, preprocess, weights, device)
...
def main():
    ...
    for i in range(len(image_paths)):
        print(image_paths[i])
        process_image(image_paths[i], center_list[i], radius_list[i], model, preprocess, weights, output_path, device)
        print("")
